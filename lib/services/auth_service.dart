...
class AuthService {
    ...
    try {
      ...
      if (qs.docs.isEmpty) {
        ...
        try {
          ...
          // If user exists in auth but not in Firestore, create profile
          try {
            UserCredential uc2 = await _auth.signInWithEmailAndPassword(
              email: adminEmail,
              password: adminPassword,
  ...
  static Future<User?> registerAdmin({
    ...
    try {
      ...
      debugPrint('Admin registered successfully: ${uc.user!.uid}');
      return uc.user;
    } on FirebaseAuthException catch (e) {
      String msg = switch (e.code) {
        'weak-password' => 'Password is too weak.',
  ...
  static Future<User?> signIn(String email, String password) async {
    try {
      ...
      // Verify admin role
      DocumentSnapshot ds = await _firestore.collection('users').doc(uc.user!.uid).get();
